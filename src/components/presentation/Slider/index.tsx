...
import useSlider from '../../../hooks/presentation/useSlider';
import { INIT, SIZE } from '../../../constants/presentation';
...
const Slider = ({ files, addFile, removeFile, passNum = 1, showNum = 3, $gap = 20, $highlight = 4 }: SliderProps) => {
  const { rootIdx, setRootIdx, position, sliderWidth, slideImage, slideRemoveImage, updateClickedHighlightIdx } =
    useSlider({
      passNum,
      showNum,
      $gap,
  ...
  const clickImageList = ({ clientX }: { clientX: number }) => {
    ...
    const xCoordinate = clientX - sliderRef!.current.getBoundingClientRect().left;
    updateClickedHighlightIdx(xCoordinate);
  };

  const lastFileIdx = files.length - 1;
  const [prevDisabled, nextDisabled] = [rootIdx === INIT.INDEX, rootIdx === lastFileIdx];

  const pageList = `${rootIdx + 1} / ${files.length}`;

  return (
    <StyledSlider $width={sliderWidth} ref={sliderRef}>
      <Image type="ROOT" file={files[rootIdx]} height={`${SIZE.ROOT_IMAGE.HEIGHT}px`} />
      <ImagelistSlider
        files={files}
        rootIdx={rootIdx}
