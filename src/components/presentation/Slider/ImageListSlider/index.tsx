...
import { SIZE } from '../../../../constants/presentation';
...
const ImageContainer = styled.div<TImageContainer>`
  ...
  img {
    ${({ $isHighlight, $highlight }) =>
      `border: ${$highlight}px solid ${$isHighlight ? 'cornflowerblue' : 'lightgray'}`};
  }
`;

type ImageListSliderProps = Pick<SliderProps, '$gap' | '$highlight' | 'files'> & {
  position: number;
...
const ImageListSlider = ({
  ...
  <ImageList $position={position} $gap={$gap} onClick={clickImageList}>
    {Object.values(files).map((file: File, fileIdx: number) => (
      ...
      >
        <Image type="ITEM" file={file} width={`${SIZE.ITEM_IMAGE.WIDTH}px`} height={`${SIZE.ITEM_IMAGE.HEIGHT}px`} />
        <span>{fileIdx + 1}</span>
      </ImageContainer>
    ))}
  </ImageList>
